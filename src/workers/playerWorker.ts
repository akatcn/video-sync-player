...
const draw = (time: DOMHighResTimeStamp) => {
  ...
  const estimatedCurrentServerTime =
    estimatedServerTime + (mainCurrentTime - mainReceivedAt);
  const estimatedFrameIndex =
    Math.floor((estimatedCurrentServerTime / 1_000) * frameRate) %
    videoFrames.length;
